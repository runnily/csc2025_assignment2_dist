 ...
 */
int do_stopfslog() {
    if (m_in.m1_i1 < FSOP_NONE || m_in.m1_i1 > FSOP_ALL) { //check in valid range for selected log operation
        return INVALID_ARG; //return invalid (-22)
    }
    //start and length fields do not have to be changed.
    fsloginf.ops2log = fsloginf.ops2log & ~(m_in.m1_i1); //else use bitwise & and operation to stop log
    return OK;
}
 ...
 */
int do_getfsloginf() {
    if (m_in.m1_p1) { //if first string is not null
        vir_bytes meta = (vir_bytes) m_in.m1_p1;
        /*copy the struct fsloginf from this system call process identified by SELF
        * into the variable meta provided by the calling process (who_e)
        * allow the size to the sizeof the struct
        */
        struct fsloginf *fslogptr = &fsloginf;
        return sys_vircopy(SELF, (vir_bytes) fslogptr, who_e, meta, sizeof(fsloginf));
 ...
 */
int do_getfslog() {
    if (m_in.m1_p1 && m_in.m1_p2) { //ensure it not null
        int r = do_getfsloginf(); //get error code
        if (r == OK) { //check if it succeeds
            vir_bytes array = (vir_bytes) m_in.m1_p2; //second string pointing to address of the calling process to store
            return sys_vircopy(SELF, (vir_bytes) fslog , who_e, array, sizeof(fslog) );
        }
        return r; //return r when it does not succeed.
    }
    return INVALID_ARG;
}
...
void logfsop(int opcode, int result, char *path, int fd_nr, mode_t omode, 
    ...
    if (fsloginf.ops2log & opcode) {
         ...
         * 
         */
        if (path) { //when path is not null
            strncpy(fslog[next].path, path, strnlen(path, PATH_MAX)); //copy string into here
        } else {
            fslog[next].path[0] = '\0';
        }
        if (fp->fp_pid) { //if fp does not equal null then find the id process
            fslog[next].cp_pid = fp->fp_pid;
        } else {
             fslog[next].cp_pid = UNKNOWN_CP_PID;
        }

        if(fp->fp_name) { //if fp calling name does not equal null 
            strncpy(fslog[next].cp_name, fp->fp_name, strnlen(fp->fp_name, PATH_MAX));
        } else {
