...
/* Perform read(fd, buffer, nbytes) or write(fd, buffer, nbytes) call. */
  ...
  /*CSC2015 mod start*/
  fsopcode = (rw_flag == READING) ? FSOP_READ : FSOP_WRITE; //set fsopcode
  /*CSC2015 mod end*/

  //get flip trys to get the filp struct with the following node, if it cannot
  //do that then we have an error
  /*CSC2015 mod start - I added the log error and the curly bracked*/
  ...
  //check if file mode matches the operation they want to do.
   /*CSC2015 mod start - the log statement and the curly brackets added*/
  if (((f->filp_mode) & (rw_flag == READING ? R_BIT : W_BIT)) == 0) {
    r = (f->filp_mode == FILP_CLOSED) ? EIO : EBADF; //r is either an invalid file descriptor for read/writing or file is closed
  ...
  if (scratch(fp).io.io_nbytes == 0) {
     /*CSC2015 mod start*/
    logfsop_nopath(fsopcode, 0, scratch(fp).file.fd_nr, f->filp_vno->v_mode, f->filp_vno->v_size);
    /*CSC2015 mod end*/
    unlock_filp(f);
  ...
  /*CSC2015 mod start - log before flip is unlocked*/
  logfsop_nopath(fsopcode, r, scratch(fp).file.fd_nr, f->filp_vno->v_mode, f->filp_vno->v_size);
  /*CSC2015 mod end*/
