...
/* Perform the link(name1, name2) system call. */
  ...
  vir_bytes vname1, vname2; //for link 1 and link 2
  size_t vname1_length, vname2_length; //for link1 and link 2
 ...
 */
  ...
  vir_bytes vname;
  size_t vname_length;
  unsigned short fsopcode = FSOP_UNLNK;

  

  vname = (vir_bytes) job_m_in.name;
  vname_length = job_m_in.name_length;
   /*CSC2025 mod start  - curly brackets added and log statement*/
  if (copy_name(vname_length, fullpath) != OK) {
    /* Direct copy failed, try fetching from user space */
    if (fetch_name(vname, vname_length, fullpath) != OK) {
        logfserr_nopath(fsopcode, err_code); //unable to find path
        return(err_code);
      }
    }
  /*CSC2025 mod end */
  ...
  if ((dirp = last_dir(&resolve, fp)) == NULL) {
    /*CSC2025 mod start */
    logfserr(fsopcode, err_code, fullpath); //unable to find path
    /*CSC2025 mod end */
    return(err_code);
  }
  ...
  if (!S_ISDIR(dirp->v_mode)) {
	...
	put_vnode(dirp);
  /*CSC2025 mod start */
    logfserr(fsopcode, ENOTDIR, fullpath); //unable to find path
  /*CSC2025 mod end */
	return(ENOTDIR);
  }
  ...
  if ((r = forbidden(fp, dirp, X_BIT | W_BIT)) != OK) {
    ...
    put_vnode(dirp);
    /*CSC2025 mod start */
    logfserr(fsopcode,r, fullpath); //unable to find path
    /*CSC2025 mod start */
    return(r);
  }
  ...
  /*CSC2025 mod start */
  if (r == OK) {
    logfsop(fsopcode,r,fullpath, UNKNOWN_FD_NR , UNKNOWN_MODE, UNKNOWN_SIZE );
   } else {
    logfserr(fsopcode, r, fullpath);    
  }
  /*CSC2025 mod end */
  return(r);
}
